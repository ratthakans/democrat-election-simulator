...
export const computeResults = (
  ...
  const rows = districts.map(({ district, party_votes: originalVotes }) => {
    const newVotes: PartyVotes = {
      ...originalVotes,
      [TARGET_PARTY]: originalVotes[TARGET_PARTY] || 0,
    }
    ...
    const { party: originalWinner, votes: originalWinnerVotes } =
      argmaxVote(originalVotes)
    const { party: newWinner } = argmaxVote(newVotes)
    ...
    return {
      ...
      status,
      votesNeeded:
        status === 'lose' ? topCompetitor - newVotes[TARGET_PARTY] + 1 : null,
      margin:
        newVotes[TARGET_PARTY] -
        (status === 'win' ? topCompetitor : argmaxVote(newVotes).votes),
